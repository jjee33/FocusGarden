...
A cozy productivity game. Every plant you grow represents real time you spent
focusing — hundreds of hours of study or work, turned into something you can look
at.

It exists twice, on purpose:

- **[thefocusgarden.com](https://thefocusgarden.com)** — the web app. Runs in the
  browser, installs as a PWA, and works offline. An account is optional and buys
  exactly one thing: the same garden on more than one device.
- **The desktop app** — Godot 4.7.1, Windows and Linux, entirely offline, no
  account of any kind.

Both read and write the same save format, so a garden can move between them.
No ads, no tracking, no monetization, nothing sold.

> **Status.** Desktop 0.1.0 is released for Windows, with the next version
> unreleased on `master`. The web app is live and in use: sign-up, email
> verification, Google sign-in, and cross-device sync all work end to end.
> See [Current state](#current-state) and [CHANGELOG.md](CHANGELOG.md).

---

## The web app

Lives in [`web/`](web/) — TypeScript, React and Vite on the client, Hono and
Drizzle on a Cloudflare Worker with D1 behind it.

**It is not a port of the Godot code; it is a port of the Godot *rules*.** Every
pure function was rewritten in TypeScript and is pinned against **4,343 fixtures
generated by the real engine running the real GDScript**, including 292 that
exist only to hold GDScript's arithmetic — `posmod`, integer division,
round-half-away-from-zero — to the behaviour JavaScript does not share. Both
clients therefore agree on what a session is worth, down to the minute.

The browser does all of it. The server stores rows and hands them back; it
computes nothing, which is what keeps it inside a free Worker's CPU budget and
what lets the whole app work with no network at all.

```bash
npm --prefix web install
npm --prefix web run dev        # Vite, port 5173
npx --prefix web wrangler dev   # the Worker and the API, port 8787
npm --prefix web test           # 194 tests
```

Auth needs credentials in `web/.dev.vars` — see
[`web/.dev.vars.example`](web/.dev.vars.example). `GET /api/health` reports which
are missing or malformed, by name and expected shape, without printing a value.
