...
### Added — Focus Garden on the web

**[thefocusgarden.com](https://thefocusgarden.com) is live.** The whole game runs
in a browser, installs as a PWA, and works with no network at all. An account is
optional and buys one thing: the same garden on more than one device.

- **The rules were ported, not the code.** Every pure function rewritten in
  TypeScript and pinned against 4,343 fixtures generated by the real engine
  running the real GDScript — including 292 that exist purely to hold GDScript's
  arithmetic to behaviour JavaScript does not share. The two clients agree on
  what a session is worth, exactly.
- **The same save format both ways.** A garden exports from the desktop, imports
  into the browser, and back again.
- **Accounts.** Email and password with verification, or Google. A verification
  email that fails to send no longer leaves an account that exists and cannot be
  used; there is a throttled resend that reports honestly when mail is refused.
- **Sync.** Server-assigned revisions, dirty-by-content-hash so no mutation can
  forget to be noticed, tombstones for deletes, and append-only set union for
  sessions so a retried push cannot double anybody's history.
- **Plants are drawn, not sprited.** Procedural SVG for all eight growth forms,
  with instanced foliage and detail that scales down as a garden fills.
- **Privacy that survives reading.** No analytics beyond a page-view count that
  is named in the policy, one strictly-necessary cookie, no tracking of any kind,
  and a complete export whenever you want it.
